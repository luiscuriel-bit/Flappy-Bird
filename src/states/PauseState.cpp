...
{
	world = _world, bird = _bird, score = _score;
	Settings::music.pause();
}

void PauseState::handle_inputs(const sf::Event& event) noexcept
{
	if (event.type == sf::Event::KeyPressed and ((event.key.code == sf::Keyboard::Escape || event.key.code == sf::Keyboard::P)))
...
void PauseState::render(sf::RenderTarget& target) const noexcept
{
	...
	render_text(target, Settings::VIRTUAL_WIDTH / 2, 2 * Settings::VIRTUAL_HEIGHT / 3, "Press P now to resume the game", Settings::MEDIUM_TEXT_SIZE, "font", sf::Color::White, true);
}